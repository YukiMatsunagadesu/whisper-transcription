...
from utils import is_video_file, extract_audio_from_video, format_timestamp
...
def main():
    ...
    st.markdown("OpenAIのWhisperモデルを使用して、音声・動画ファイルからテキストへの文字起こしを行います。")
    ...
    input_method = st.radio(
        "入力方法を選択",
        ["ファイルアップロード（〜500MB）", "ローカルファイルパスを指定（大容量動画向け）"],
        horizontal=True
    )
    ...
    if input_method == "ファイルアップロード（〜500MB）":
            ...
            with st.spinner("文字起こし処理中..."):
                ...
                try:
                    ...
                    if is_video_file(base_filename):
                        temp_audio = extract_audio_from_video(source_path, progress_text.text)
                    ...
                    progress_text.text(f"モデルロード完了（{time.time() - load_start:.2f}秒）")
                    ...
                    with st.expander("詳細（タイムスタンプ付き）"):
                        ...
                        for segment in result["segments"]:
                            ...
                            table_data.append({"開始": start_fmt, "終了": end_fmt, "テキスト": segment["text"]})
                            timestamp_text += f"[{start_fmt} --> {end_fmt}] {segment['text']}\n"
                    ...
                    for f in (temp_input, temp_audio):
                        if f:
                            try:
                                os.unlink(f)
                            except OSError:
